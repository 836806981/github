...
//        $admin_user = M('permission')->where('employee_id='. $_SESSION[C('TRAINING_AUTH_KEY')]['id'].'')->find();
        if($_SESSION[C('TRAINING_AUTH_KEY')]['permission']!=1){
    ...
    public function login(){
        if(I('post.')) {
            ...
            if ($user_info && $user_info != '') {//登录成功的处理
                $admin_user = M('permission')->field('edit_per,add_per,del_per,nurse,news,job,destine,trainee,training,order')->where('employee_id='.$user_info['id'] .'')->find();
                if($user_info['status']==0||$admin_user['training']!=1){
                    echo "<script>alert('账号被停用或无权限，无法登陆！');window.location.href='" . __MODULE__ . "/Admin/login.html';</script>";
                }else {

                    $user_info = array_merge($user_info,$admin_user);
                    $_SESSION[C('TRAINING_AUTH_KEY')] = $user_info;
                    ...
                    echo "<script>alert('登录成功！');window.location.href='" . __MODULE__ . "/Training/tlist/type/6.html';</script>";
                }
            } else{
                $ip = $this->getIP();
                $log['sessionid']='';
    ...
    public function info(){
        ...
        if(I('post.')){
            ...
            $user_info = M('employee')->where('id=' . $_SESSION[C('TRAINING_AUTH_KEY')]['id'] . '')->find();
            $login_time = M('login_log')->field('login_time')->where('employee_id=' . $_SESSION[C('TRAINING_AUTH_KEY')]['id'] . '')->order('login_time desc')->limit(2)->select();
    ...
    public function changepass(){
        ...
        if(I('post.')){
            $post = I('post.');
            $user = M('employee')->where('id=' . $_SESSION[C('TRAINING_AUTH_KEY')]['id'] . '')->find();
            if($post['old']==''||$post['new']==''||$post['newre']==''){
                ...
                $where['id']=$_SESSION[C('TRAINING_AUTH_KEY')]['id'];
    ...
    public function login_out(){
        $_SESSION[C('TRAINING_AUTH_KEY')]=null;
