...
$.extend({
    ...
    getNewsListByAllTerm: function () {
        ...
        $.AjaxPost(MODULE+"/Goal/get_email_list", {type:type,currentpage:currentpage_zcj, pagenum:pagenum_zcj}, function (backdata) {
            if (backdata.code == 1000) {
				...
				if(List_zcj!=null){
                    if(backdata.data.type==1) {
                        ...
                        <td>' + (item.send_email==1?'是':'否')+ '</td>\
                        <td><a href="' + MODULE + '/Goal/send_user_id/id/' + item.id + '/type/1">'+(item.send_email==1?'再次提醒':'提醒')+'</a></td>\
                        </tr>';
                        ...
                        $.each(List_zcj, function (i, item) {
                            ...
                            <td>' + item.status_name+ '</td>\
                            <td>' + (item.send_email==1?'是':'否')+ '</td>\
                            <td><a href="' + MODULE + '/Goal/send_user_id/id/' + item.id + '/type/2">'+(item.send_email==1?'再次提醒':'提醒')+'</a></td>\
                        </tr>';
                        });
                    }
                    $dom.append(str);
