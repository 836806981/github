...
$.extend({
    ...
    getNewsListByAllTerm: function () {
        ...
        $.AjaxPost(MODULE+"/Goal/getProject", {user_id:user_id,status:status,currentpage:currentpage_zcj, pagenum:pagenum_zcj}, function (backdata) {
            if (backdata.code == 1000) {
				...
				if(List_zcj!=null){
                    $.each(List_zcj,function(i,item){
                        var edit_str = '';
                        if(item.status<3&&item.is_boss==1){
                            edit_str += '<a href="'+MODULE+'/Goal/edit_project/id/'+item.id+'">修改</a> | ';
                        }
                        str +='<tr>\
                        <td>'+item.name+'</td>\
                        ...
                        <td>'+item.status_name+(item.time_true==1?'(超时)':'')+'</td>\
                        <td>'+edit_str+'<a href="'+MODULE+'/Goal/project_info/id/'+item.id+'">查看详情</a></td>\
                        </tr>';
