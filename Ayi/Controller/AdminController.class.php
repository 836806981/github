...
//        $admin_user = M('permission')->where('employee_id='. $_SESSION[C('AYI_AUTH_KEY')]['id'].'')->find();
        if($_SESSION[C('AYI_AUTH_KEY')]['permission']!=1){
    ...
    public function test(){
        ...
        $save['money'] = array('exp','money+5');
        M('tra_stay')->where('id=12')->setInc($save);
    ...
    public function login(){
        if(I('post.')) {
            ...
            if ($user_info && $user_info != '') {//登录成功的处理
                ...
                if($user_info['status']==0||$admin_user['nurse']!=1){
                    echo "<script>alert('账号被停用或非阿姨管理账号，无法登陆！');window.location.href='" . __MODULE__ . "/Admin/login.html';</script>";
                }else {

                    $user_info = array_merge($user_info,$admin_user);
                    $_SESSION[C('AYI_AUTH_KEY')] = $user_info;
    ...
    public function info(){
        ...
        if(I('post.')){
            ...
            $user_info = M('employee')->where('id=' . $_SESSION[C('AYI_AUTH_KEY')]['id'] . '')->find();
            $login_time = M('login_log')->field('login_time')->where('employee_id=' . $_SESSION[C('AYI_AUTH_KEY')]['id'] . '')->order('login_time desc')->limit(2)->select();
    ...
    public function changepass(){
        ...
        if(I('post.')){
            $post = I('post.');
            $user = M('employee')->where('id=' . $_SESSION[C('AYI_AUTH_KEY')]['id'] . '')->find();
            if($post['old']==''||$post['new']==''||$post['newre']==''){
                ...
                $where['id']=$_SESSION[C('AYI_AUTH_KEY')]['id'];
