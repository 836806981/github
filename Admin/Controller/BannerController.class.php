...
class BannerController extends Controller {
    ...
    public function bannerList(){
        ...
        $banner_list = M('banner')->order('belong asc,status asc,sort desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $status_name = ['','显示','不显示'];
    ...
    public function addBanner(){
        $sort = M('banner')->field('sort')->order('sort desc')->limit(1)->find();
    ...
    public function editBanner(){
        if(is_numeric(I('get.id'))){
            $banner = M('banner')->where('id='.I('get.id').'')->find();
            if($banner&&$banner!=''){
    ...
    public function edit_banner(){
        ...
        $add_mod = M('banner')->where('id='.$post['id'])->save($post);
    ...
    public function deleteBanner(){
        ...
        $info = M('banner')->where('id='.I('get.id').'')->find();
        if(!$info){
            echo "<script>alert('地址异常');history.back();</script>";
            exit;
        }
        $delete = M('banner')->where('id='.I('get.id').'')->delete();
        if($delete!==false){
    ...
    public function friendList(){
        $count = M('friendship_link')->count();
        $Page = new \Think\Page($count,20);// 实例化分页类 传入总记录数和每页显示的记录数(25)
        $show = $Page->showFront();// 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性

        $friend_list = M('friendship_link')->order('status asc,sort desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $status_name = ['','显示','不显示'];
        $target_name = ['','是','否'];
    ...
    public function addFriend(){
        $sort = M('friendship_link')->field('sort')->order('sort desc')->limit(1)->find();
        $sort_num = $sort['sort']?$sort['sort']+1:100;
    ...
    public function add_friend(){
        $post = I("post.");
        $add_mod = M('friendship_link')->add($post);
        if($add_mod!==false){
    ...
    public function editFriend(){
        if(is_numeric(I('get.id'))){
            $friend = M('friendship_link')->where('id='.I('get.id').'')->find();
            if($friend&&$friend!=''){
    ...
    public function edit_friend(){
        $post = I("post.");
        $add_mod = M('friendship_link')->where('id='.$post['id'])->save($post);
    ...
    public function deleteFriend(){
        ...
        $info = M('friendship_link')->where('id='.I('get.id').'')->find();
        if(!$info){
            echo "<script>alert('地址异常');history.back();</script>";
            exit;
        }
        $delete = M('friendship_link')->where('id='.I('get.id').'')->delete();
        if($delete!==false){
