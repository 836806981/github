...
//        $admin_user = M('n_permission')->where('employee_id='. $_SESSION[C('N_ADMIN_AUTH_KEY')]['id'].'')->find();
        ...
        if(I('post.')) {
            ...
            if ($user_info && $user_info != '') {//登录成功的处理
                if($user_info['status']==0){
                    ...
                    $admin_user = M('n_permission')->field('edit_per,add_per,del_per,nurse,news,job,destine,trainee,order,video,banner,friend_ship,pay')->where('employee_id='.$user_info['id'] .'')->find();
                    $user_info = array_merge($user_info,$admin_user);

                    $_SESSION[C('N_ADMIN_AUTH_KEY')] = $user_info;
    ...
    public function addEmployee(){
        ...
        if (I('post.')) {
            $post = I('post.');
            $username = M('n_employee')->where('username="'.$post['username'].'"')->find();
            if($username&&$username!=''){
                ...
                if( $post['permission']==1){
                    ...
                    $permission['friend_ship'] = $post['friend_ship'];
                    $permission['banner'] = $post['banner'];
                    $permission['pay'] = $post['pay'];

                }
                $permission_id = M('n_permission')->add($permission);
                if ($employee_id && $permission_id) {
                    $back['code']=1000;
                } else {
                    $where_per['id']=$permission_id;
                    $where_emp['id']=$employee_id;
                    M('n_permission')->delete($where_per);
                    M('n_employee')->delete($where_emp);
                    $back['code']=1001;
                }
            }
    ...
    public function set_status(){
        ...
        $save_mod =  M('n_employee')->where($where)->save($map);
        if($save_mod!==false){
    ...
    public function info(){
        ...
        if(I('post.')){
            ...
            $employee = M('e_employee');
            $save_mod = $employee->where($where)->save($info);
            if($save_mod!==false){
            ...
            if(I('get.id')){
                if($_SESSION[C('N_ADMIN_AUTH_KEY')]['permission']==1){
                    $user_info = M('n_employee')->where('id=' .I('get.id'). '')->find();
                }else{
                    if(I('get.id')==$_SESSION[C('N_ADMIN_AUTH_KEY')]['id']){
                        $user_info = M('n_employee')->where('id=' .I('get.id'). '')->find();
                    }else{
                ...
                $user_info = M('n_employee')->where('id=' . $_SESSION[C('N_ADMIN_AUTH_KEY')]['id'] . '')->find();
            }
            $per = array('','超级管理员','普通管理员','阿姨管理员');
            $user_info['per'] = $per[$user_info['permission']];
            $login_time = M('login_log')->field('login_time')->where('employee_id=' . $_SESSION[C('N_ADMIN_AUTH_KEY')]['id'] . '')->order('login_time desc')->limit(2)->select();
            $user_info['log_time'] = $login_time[1]['login_time'] ? date('Y-m-d H:i:s', $login_time[1]['login_time']) : '首次登录';
            $permission = M('n_permission')->where('employee_id='.$user_info['id'].'')->find();
    ...
    public function changeper(){
        ...
        if($_SESSION[C('N_ADMIN_AUTH_KEY')]['permission']!=1){
            ...
            $post['banner']||$post['banner']=0;
            $post['friend_ship']||$post['friend_ship']=0;
            $post['pay']||$post['pay']=0;
            $where['employee_id']=$post['employee_id'];
            $save_mod = $permission ->where($where)->save($post);
            if($save_mod!==false){
    ...
    public function changepass(){
        ...
        if(I('post.')){
            $post = I('post.');
            $user = M('n_employee')->where('id=' . $_SESSION[C('N_ADMIN_AUTH_KEY')]['id'] . '')->find();
            if($post['old']==''||$post['new']==''||$post['newre']==''){
                ...
                $where['id']=$_SESSION[C('N_ADMIN_AUTH_KEY')]['id'];
                $info['pwd']=$this->md5_pwd($post['new']);
                $save = M('n_employee')->where($where)->save($info);
                if($save!==false){
                    $back['code']=1000;//修改成功
                }else{
                    $back['code']=1004;//修改失败
    ...
    public function login_out(){
        $_SESSION[C('N_ADMIN_AUTH_KEY')]=null;
    ...
    public function video(){
        if($_SESSION[C('N_ADMIN_AUTH_KEY')]['video']!=1){
            echo "<script>alert('你没有权限！');window.location.href='" . __MODULE__ . "/Admin/info.html';</script>";
            exit;
        }
