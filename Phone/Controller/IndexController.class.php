...
//            12880=>'超级田螺12880元,六大基础护理+4*儿保+PICC百万保险一份+产房监测+待产包+产后美容+中医通乳+2*孕检陪护+ 营养师定制食谱+心理疏导,超级田螺，50个以上宝宝护理经验。服务周期：26天');
        ...
        $total_fee = 0.01;//$alipay['total_fee'];//$_POST['ordtotal_fee'];   //付款金额  //必填 通过支付页面的表单进行传递
        $body = $alipay['body'];//$_POST['ordbody'];  //订单描述 通过支付页面的表单进行传递
        $show_url = 'http://www.tianluoayi.com/m/yuesao.html';  //商品展示地址 通过支付页面的表单进行传递
...
//如要调试，请看alipay_notify.php页面的verifyReturn函数
    ...
    public function index(){
        ...
        $news_list = M('n_news')->where('recommend=1')->order('sort desc')->limit(0,3)->select();
...
//        $this->assign('banner',$banner);
    ...
    public function newsEventCnt(){
        if(I('get.id')&&I('get.id')!=''&&is_numeric(I('get.id'))){
            ...
            if(!$news_info||$news_info==''){
                echo "<script>alert('地址异常');window.location.href='".__ROOT__."/m/';</script>";
                exit;
            }
    ...
    public function newsCnt(){
        ...
        $nav_name_url = '<a href="'.__ROOT__.'/m/'.$belong_url.'.html">'.$nav_name[$news_info['belong']].'</a>&gt;';
        ...
        if(I("get.type")){
            ...
            if($article_type[I("get.type")]<=6&&$article_type[I("get.type")]>0){
                ...
                if($keywords['type']==0){//是搜索的页面
                    ...
                    $nav = '<a href="'.__ROOT__.'/m/yuesao.html">月嫂</a>&gt;'.$nav_arr[$article_type[I("get.type")]];
                }
...
//            }
    ...
    public function ys(){
        ...
        $this->display('Index/Auction/shouji-yuesao');
    }

    public function yes(){

        $this->display('Index/Auction/shouji-yuersao');
    }

    public function bm(){

        $this->display('Index/Auction/shouji-baomu');
    }

    public function zp(){

        $this->display('Index/Auction/shouji-zhaopin');
    }
