...
class IndexController extends CommonController {
    ...
    public function index(){
        //首页新闻展示
        $news_list = M('news')->where('belong=1 and index_show=1')->order('sort desc')->limit(0,3)->select();
...
//        $this->assign('page',$show);
    ...
    public function addMessage(){
        ...
        if($post['name']==''||$post['phone']){
            echo "<script>alert('请填写姓名和电话');history.go(-1);</script>";
        }
...
//            $receiver = '836806981@qq.com,huangw@tianluoayi.com';
    ...
    public function addDestine(){
        ...
        $post['add_time']=time();
        if($post['phone']==''){
            echo "<script>alert('请填写手机号！');history.back();</script>";
        }
